...
const TheaterListsByCinema = ({ cinemas, selectedCinemaIndex, setSelectedCinemaIndex, fetchCinemas, auth }) => {
	...
	const handleDelete = (cinema) => {
		const confirmed = window.confirm(
			`Do you want to delete Theater ${cinema.name}, including its screens, showtimes and tickets?`
		)
		if (confirmed) {
	...
	const onDeleteCinema = async (id) => {
		try {
			...
			fetchCinemas()
			toast.success('Delete Theater successful!', {
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false
			})
	...
	const onIncreaseTheater = async (data) => {
		try {
			...
			fetchCinemas()
			// console.log(response.data)
			toast.success('Add Screens successful!', {
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false
			})
	...
	const onEditCinema = async (data) => {
		try {
			...
			toast.success('Edit Theaters name successful!', {
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false
			})
							...
							<div className="flex grow flex-col gap-4 sm:justify-end md:flex-row">
								...
								<div className="flex grow md:grow-0">
									<div className="flex flex-col items-center justify-center gap-1 rounded-l bg-gradient-to-br from-gray-800 to-gray-700 p-1 text-white">
										<label className="text-xs font-semibold leading-3">Screen</label>
